...
public abstract class AbstractClient extends AbstractEndPoint implements Client {
    ...
    public AbstractClient(URL url, ChannelHandler handler) throws RemotingException {
        ...
        reconnect_warning_period = 1800;
        try {
            doOpen();
        } catch (Throwable t) {
            close();
            throw new RemotingException(new InetSocketAddress(getUrl().getHost(), getUrl().getPort()), null,
            ...
            Runnable connectStatusCheckCommand =  new Runnable() {
                public void run() {
                    try {
                        ...
                        String errorMsg = "client reconnect to "+getUrl().getHost()+" find error . url: "+ getUrl();
    ...
    protected ExecutorService createExecutor() {
        return Executors.newCachedThreadPool(new NamedThreadFactory(CLIENT_THREAD_POOL_NAME + CLIENT_THREAD_POOL_ID.incrementAndGet() + "-" + getUrl().getHost(), true));
    }
    ...
    public void send(Object message, boolean sent) throws RemotingException {
        ...
        if (channel == null || ! channel.isConnected()) {
          throw new RemotingException(getChannel(), "message can not send, because channel is closed . url:" + getUrl());
        }
